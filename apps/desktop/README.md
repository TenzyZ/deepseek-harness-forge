...
The PIN cannot contain `]`, a quote, or a line break because those characters delimit the SafeNet `/kc` value or its CMD argument. The CMD disables delayed expansion so a PIN containing `!` reaches SafeNet unchanged. Packaging withholds every `DSH_DESKTOP_WINDOWS_*` field from build and seed-preparation subprocesses, gives electron-builder only the six approved signing input names that are present, gives the signing CMD only the validated production fields in an otherwise scrubbed environment, clears those fields before SignTool starts, and redacts SignTool diagnostics. SafeNet still requires the PIN in the SignTool process command line. Inject it as an ephemeral secret only on a controlled self-hosted Windows runner with the physical token attached; never commit it, put it in `.env`, or persist it as a Windows user or system environment variable.

#### Local test signing

Set `DSH_DESKTOP_WINDOWS_SIGNING_ENV=local-test` to select a dedicated Code Signing certificate from `Cert:\CurrentUser\My`, and set `DSH_DESKTOP_WINDOWS_TEST_CERT_SHA1` to its exact 40-hexadecimal-character SHA-1 thumbprint. This explicit local development and test mode also requires the validated `DSH_DESKTOP_WINDOWS_SIGNTOOL`; it rejects the production certificate file, key container, and Token Password inputs instead of falling back between signing modes.

Local-test signing invokes SignTool directly with the fixed current-user `My` store, SHA-256 file digest, selected thumbprint, and no timestamp server. It does not use a PFX file or password, load or export the private key, or modify a trust store. The dedicated certificate and its private key must remain outside the repository. Its signature is not publicly trusted and is not a release-signing mechanism; later operator verification must confirm the installer certificate and thumbprint without installing the certificate in a Trusted Root store.
