...
Windows 签名遵循[本地测试签名决策](2026-09-10-dsh-forge-windows-local-test-signing.zh.md)：SafeNet 硬件 Token SignTool 路径仍为生产默认路径，显式的本地测试模式则使用当前用户 `My` 存储中的专用证书。签名前诊断验证仍通过 'prepare:desktop' 运行，无需任一签名身份。
...
- **公开 Windows 签名提供者与证书**：尚未选定受信任的公开签名提供者或证书。SafeNet 硬件 Token SignTool 路径仍为生产默认路径，本地测试证书不能用于公开发布资格验证。
