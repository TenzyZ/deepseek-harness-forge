...
Windows signing follows the [local-test signing decision](2026-09-10-dsh-forge-windows-local-test-signing.md): the SafeNet hardware-token SignTool path remains the production default, while explicit local-test mode uses a dedicated certificate in the current user's `My` store. Pre-sign diagnostic verification still runs through 'prepare:desktop' without requiring either signing identity.
...
- **Public Windows signing provider and certificate**: no trusted public provider or certificate is selected. The SafeNet hardware-token SignTool path remains the production default, and the local-test certificate does not qualify a public release.
