...
The model picker offers an **Effort** menu for a model that declares reasoning levels. A built-in provider's models inherit their levels from the installed catalog. A model you enter by hand declares none until you say what it supports, so the Effort entry does not appear in the menu and the endpoint's own default decides whether the model thinks. Declare the levels on the Models page: edit the provider, open the model row's **Model capabilities** disclosure, set **Supported reasoning efforts** to **Choose supported levels**, and tick each level the model accepts — giving a level its gateway spells differently that spelling, and leaving `off` empty to send no reasoning field at all. **Use catalog capability** returns the model to whatever the installed catalog declares, and **No reasoning support** takes its levels away. The page writes the same `reasoningEfforts` field you can write in `$DSH_HOME/settings.yaml` instead:
...
Each key is a level the menu offers, and its value is the spelling sent on the wire as `reasoning_effort`, so `max: xhigh` renames a level for a gateway with its own vocabulary. Only `off` may stay empty, because for most endpoints not thinking is the parameter's absence. The route's `reasoning` is the level used while a session has picked none; choosing an effort in the picker saves it, with the model, as the default for new sessions. A declaration states a claim about your model rather than checking it: nothing interrogates a gateway for the levels it accepts, so declare only the ones that model really serves — a level it rejects fails the request that names it.
...
- **The Effort menu does not appear for a model you entered by hand** — It declares no levels. Declare them in the model's **Model capabilities** disclosure on the Models page, or add `reasoningEfforts` to the model in `settings.yaml`.
