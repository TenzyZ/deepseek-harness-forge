...
::: tip 表单刻意保持精简
模型页开放让一条路由得以存在的字段——API 密钥、显示名称、API 地址、API 协议，以及每个模型的 ID、显示名称、上下文窗口和最大输出 token 数——外加每个模型支持的推理等级。其余所有字段——图片输入、请求兼容性开关、请求头、超时、重试策略——都在 `$DSH_HOME/settings.yaml` 中设置，也就是模型页写入的同一份文档。可以直接编辑它；浏览器与服务器在同一台机器时，也可以点击设置页顶部的**打开配置文件**打开它。适配器会在下一次请求时重新读取，无需重启任何东西。下面各小节介绍多数网关会用到的字段。
:::
...
对于声明了推理等级的模型，模型选择器会提供**推理等级**菜单。内置提供方的模型从已安装目录继承其等级。手动录入的模型在你说明它支持什么之前不声明任何等级，因此模型菜单里不会出现推理等级项，由端点自身的默认值决定模型是否思考。可在模型页声明等级：编辑该提供方，展开模型行的**模型能力**折叠区，将**支持的推理等级**设为**选择支持的等级**，并勾选该模型接受的每个等级——网关写法不同的等级填入其写法，`off` 留空则不发送任何推理字段。**使用目录能力**使该模型回到已安装目录所声明的能力，**不支持推理**则去掉它的等级。模型页写入的正是你也可以改在 `$DSH_HOME/settings.yaml` 中写的同一个 `reasoningEfforts` 字段：
...
每个键都是菜单提供的一个等级，其值是在协议上以 `reasoning_effort` 发送的写法，因此 `max: xhigh` 可以为自有一套词汇的网关重命名某个等级。只有 `off` 可以留空，因为对多数端点来说，不思考就是不传该参数。路由的 `reasoning` 是会话尚未选择等级时采用的等级；在选择器中选定某个等级后，它会与模型一起保存为新会话的默认值。声明只是对你的模型做出断言，而不会去核实：没有任何机制会向网关询问它接受哪些等级，因此只声明该模型真正提供的等级——它不接受的等级会让指定该等级的请求失败。
...
- **手动录入的模型没有推理等级菜单**：该模型没有声明任何等级。请在模型页该模型的**模型能力**折叠区声明，或在 `settings.yaml` 中给该模型加上 `reasoningEfforts`。
